...
<Callout title="Note">
Only enabled portal payment methods appear as options when [creating orders](/docs/orders/creating-orders) in the tenant portal.
</Callout>
...
|--------|---------------------|
| **Cash** | Customer selects "Cash" at checkout. A pending payment record is created. You mark it as paid from the tenant portal when the customer pays on delivery or pickup. |
| **Receipt** | Customer selects "Upload Receipt" and uploads a photo of their payment transfer. A pending payment is created with the receipt attached. You review and accept the receipt from the tenant portal. |
