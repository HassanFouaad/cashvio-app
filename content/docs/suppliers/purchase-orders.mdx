...
You can also start a purchase order directly from [Inventory > Low Stock](/docs/inventory/low-stock-alerts): open the row's actions menu and choose **Create Purchase Order**. The form opens with the store preselected and the item already added, with a **suggested restock quantity** and the variant's **purchase price** as the unit cost. You only pick the supplier and submit.
