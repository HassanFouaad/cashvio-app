...
<Callout title="Note">
Facebook Pixel only works on your **online storefront**. It does not track actions in the business portal (admin dashboard). The Pixel is loaded per store, so each store can have its own Pixel ID.
</Callout>
