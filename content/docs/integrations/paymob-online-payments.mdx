...
2. On the **Paymob** card, click **Connect**.
3. Choose the **Store** to connect.
4. Choose the **Country** of your Paymob account. The country must match your store currency: Egypt (EGP), UAE (AED), Saudi Arabia (SAR), or Oman (OMR).
5. Paste the **API Key**, **Secret Key**, **Public Key**, and **HMAC Secret**.
6. Add your **Integration IDs**, one entry per payment method.
7. Optionally set a **Payment fee** (see below).
8. Save, then click **Verify connection** to test your keys live against Paymob.
...
| Setting | Options |
|---------|---------|
| **Type** | Percentage of the order total, or a fixed amount |
| **Direction** | Increase (surcharge added to the total) or Decrease (discount off the total) |
...
The storefront checkout now shows **Pay Online**. You cannot enable Online Payment before connecting a gateway, and you cannot disable or delete the gateway while Online Payment is still enabled.
...
| Problem | Fix |
|---------|-----|
| **Verify connection fails on the API key or Secret key** | Re-copy the key from Paymob, Settings > API Keys. Make sure there are no extra spaces. |
| **Verify connection fails on the Integration IDs** | Your keys and Integration IDs are from different modes (test vs live) or a different country account. Match them and try again. |
