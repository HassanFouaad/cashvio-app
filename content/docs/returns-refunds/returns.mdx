...
Open a return with **Return details** or its return number link. **Return overview** shows the linked order, refund total, status, and return reason when present. Returned items and supporting information follow below.

Use the back arrow at the top to return to the same search, filters, and page on **Sales > Returns**. When you open a return from an order, the back arrow returns to that order instead. Direct links fall back to **Sales > Returns**.
